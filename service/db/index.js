...
const query = (sql, value) => new Promise((resolve, reject) => {
...
/* 用户相关 */
exports.signup = value => query('insert into users set name=?,password=?;', value);

exports.findUserByname = name => query(`select name,password from users where name='${name}';`);
...
exports.selectPermissionById = id => query(`select name from permissions where FIND_IN_SET(_id, '${id}')`);

exports.searchUserRolesByUser = id => query(`select role_id from users_role where user_id=${id}`);

exports.searchUserPermissionByRole = id => query(`select permission_id from roles_permission where role_id=${id}`);

/*
 *文章相关
 */
exports.posts = (offset, limit) => query(
  `select * from posts ORDER BY created desc LIMIT ${(offset - 1)
      * limit},${limit};`,
);

exports.publishPost = value => query(
...
exports.comment = value => query(
  'insert into comments set authorId=?,author=?,ownerId=?,text=?;',
  value,
);

exports.updatePostCommentsNum = (pid, action) => query(`select commentsNum from comments where pid=${pid}`).then((_) => {
